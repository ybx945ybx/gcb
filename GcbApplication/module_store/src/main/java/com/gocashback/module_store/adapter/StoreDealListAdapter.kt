 ...
 */
class StoreDealListAdapter(private val mActivity: Activity?, list: List<DealItemModel>) : BaseQuickAdapter<DealItemModel, BaseViewHolder>(R.layout.item_deal, list) {

    override fun convert(helper: BaseViewHolder, item: DealItemModel) {
        with(item) {
            helper
                    .setText(R.id.tv_discount_country, store)
